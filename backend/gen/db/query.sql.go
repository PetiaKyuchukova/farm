...
SELECT id, birthdate, farmerid, colour, motherid, lastovulation, lastbirth, ispregnant, fertilization, givingbirthdate FROM cows
...
const getNotificationsByDate = `-- name: GetNotificationsByDate :many
SELECT id, cowid, date, type, text FROM notifications
where date =$1
`

func (q *Queries) GetNotificationsByDate(ctx context.Context, date time.Time) ([]Notification, error) {
	rows, err := q.query(ctx, q.getNotificationsByDateStmt, getNotificationsByDate, date)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
INSERT INTO notifications(cowID,date,type, text) VALUES ($1, $2, $3,$4)
`
